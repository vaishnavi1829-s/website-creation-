...
export default function ComedyShows() {
  ...
  return (
    <div className="comedy-shows-page">
      {/* Hero Banner */}
      <div className="event-banner event-banner-comedy">
        <div className="event-banner-content animate-fade-in">
          <h1>😂 Comedy Shows</h1>
          <p>Laugh out loud with India's funniest comedians — stand-up specials, open mics, improv shows & more</p>
        </div>
      ...
      <div className="events-section">
            ...
            {featured.map(e => (
              <div key={e.id} className="event-highlight-card">
                ...
                <div className="event-highlight-overlay">
                  <h3>{e.title}</h3>
                  <p>
                    {e.artist_name}{e.rating ? ` • ⭐ ${e.rating}` : ''} • ₹{e.price.toLocaleString('en-IN')}
                  </p>
                </div>
              </div>
            ))}
        ...
        <div className="event-filters animate-fade-in-up">
          <div className="event-filter-group" style={{ flex: 2, minWidth: 200 }}>
            <label>Search Comedians & Shows</label>
            <input
              type="text"
              placeholder="Search by comedian, show name, venue..."
              value={search}
        ...
        <h2 className="events-section-title">
          {hasFilters ? `Search Results (${filteredEvents.length})` : '🎤 All Comedy Events'}
        </h2>

        {filteredEvents.length === 0 ? (
          <div className="event-empty">
            <span className="event-empty-icon">🎭</span>
            <h3>No comedy events found</h3>
            <p>Try adjusting your search or filters to find the perfect comedy show!</p>
          </div>
        ) : (
